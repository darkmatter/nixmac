...
describe('conversational prompts', () => {
  registerPromptSuiteBeforeEach({
    fixtureByTestTitle: {
      'submits a conversational prompt on the Begin step':
        getMockVllmFixturePreset('conversationalPromptsOnBegin'),
      'submits a conversational prompt on the Evolve step':
        getMockVllmFixturePreset('conversationalPromptsOnEvolve'),
    },
  });
