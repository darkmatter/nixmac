# programs.htop

<!-- AUTO-GENERATED by scripts/nix-options.sh. Do not edit by hand. -->

All options under `programs.htop`.

| Option | Type | Description |
| --- | --- | --- |
| `programs.htop.enable` | `boolean` | Whether to enable htop. |
| `programs.htop.package` | `package` | The htop package to use. |
| `programs.htop.settings` | `attribute set of (boolean or signed integer or string or list of (signed integer or string))` | Configuration options to add to {file}`$XDG_CONFIG_HOME/htop/htoprc`. |
