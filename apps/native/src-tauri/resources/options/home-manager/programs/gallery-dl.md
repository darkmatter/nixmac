# programs.gallery-dl

<!-- AUTO-GENERATED by scripts/nix-options.sh. Do not edit by hand. -->

All options under `programs.gallery-dl`.

| Option | Type | Description |
| --- | --- | --- |
| `programs.gallery-dl.enable` | `boolean` | Whether to enable gallery-dl. |
| `programs.gallery-dl.package` | `null or package` | The gallery-dl package to use. |
| `programs.gallery-dl.settings` | `JSON value` | Configuration written to {file}`$XDG_CONFIG_HOME/gallery-dl/config.json`. See <https://github.com/mikf/gallery-dl#configuration> for supported values. |
