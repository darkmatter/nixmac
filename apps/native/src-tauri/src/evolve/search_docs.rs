...
/// This is included as a static string for simplicity and speed of access.
/// The search_docs tool parses this JSON into an in-memory index on first use,
...
pub fn execute_search_docs(query: &str, limit: usize) -> Result<String> {
    ...
    if ranked.is_empty() {
        return Ok(format_no_results_message(query));
    }
...
fn format_no_results_message(query: &str) -> String {
    format!(
        "{}: query='{}'. No nix-darwin option paths matched this query. Treat this as definitive and do not retry with similar wording or different limits.",
        NO_RESULTS_SENTINEL, query
    )
}
...
mod tests {
    ...
    fn no_results_message_has_sentinel_and_query() {
        ...
        let response = execute_search_docs(query, 3).expect("search_docs should succeed");
