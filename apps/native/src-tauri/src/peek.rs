...
// =============================================================================
// Rebuild Overlay Window
// =============================================================================

/// Track if the rebuild overlay is currently visible
static REBUILD_OVERLAY_VISIBLE: AtomicBool = AtomicBool::new(false);

/// Creates the rebuild overlay window (internal implementation).
/// This is a full-screen transparent overlay shown during nix-rebuild switch.
/// MUST be called from the main thread.
fn create_rebuild_overlay_window_impl(app: &AppHandle) -> Result<(), String> {
    // Get the primary monitor for full-screen dimensions
    let monitor = get_primary_monitor(app)?;
    ...
    peek_log!(
        "🔄 Creating rebuild overlay window: {}x{} at ({}, {}), scale={}",
        logical_width,
        logical_height,
        logical_x,
        logical_y,
    ...
    let window = WebviewWindowBuilder::new(
        app,
        "rebuild-overlay",
        WebviewUrl::App("rebuild-overlay.html".into()),
    )
    ...
    // Configure as an overlay panel on macOS
    #[cfg(target_os = "macos")]
    {
        use cocoa::appkit::{NSWindow, NSWindowCollectionBehavior};
        use cocoa::base::id;
        ...
        if let Ok(ns_window) = window.ns_window() {
            let ns_win = ns_window as id;
            unsafe {
                // Make it visible on all spaces and not part of window cycling
                ns_win.setCollectionBehavior_(
                    NSWindowCollectionBehavior::NSWindowCollectionBehaviorCanJoinAllSpaces
                        | NSWindowCollectionBehavior::NSWindowCollectionBehaviorStationary
                        | NSWindowCollectionBehavior::NSWindowCollectionBehaviorIgnoresCycle,
                ...
                // Set as a floating panel level (above regular windows)
                let floating_level: i64 = 5; // kCGFloatingWindowLevel
                let _: () = msg_send![ns_win, setLevel: floating_level];

                // Ignore mouse events so clicks pass through to windows below
                // (except for the content area which handles its own events)
...
/// Shows the rebuild overlay window.
/// This function dispatches to the main thread since macOS AppKit requires it.
pub fn show_rebuild_overlay(app: &AppHandle) -> Result<(), String> {
    peek_log!("🔄 show_rebuild_overlay called");

    let app_clone = app.clone();

    // Use run_on_main_thread to ensure AppKit operations happen on the main thread
    app.run_on_main_thread(move || {
        peek_log!("🔄 show_rebuild_overlay running on main thread");

        // Create the window if it doesn't exist
        if app_clone.get_webview_window("rebuild-overlay").is_none() {
            peek_log!("🔄 Creating rebuild overlay window...");
            if let Err(e) = create_rebuild_overlay_window_impl(&app_clone) {
                peek_log!("🔄 ❌ Failed to create rebuild overlay: {}", e);
                return;
            }
            peek_log!("🔄 ✓ Rebuild overlay window created");
        }

        if let Some(window) = app_clone.get_webview_window("rebuild-overlay") {
            // Reposition to cover the primary screen
            if let Ok(monitor) = get_primary_monitor(&app_clone) {
                let scale = monitor.scale_factor;
                let logical_x = monitor.x / scale;
                let logical_y = monitor.y / scale;
                let phys_x = (logical_x * scale) as i32;
                let phys_y = (logical_y * scale) as i32;

                let _ = window.set_position(PhysicalPosition::new(phys_x, phys_y));
            }
            let _ = window.show();
            REBUILD_OVERLAY_VISIBLE.store(true, Ordering::SeqCst);
            peek_log!("🔄 Showing rebuild overlay");
        }
    })
    .map_err(|e| e.to_string())?;
...
/// This function dispatches to the main thread since macOS AppKit requires it.
pub fn hide_rebuild_overlay(app: &AppHandle) -> Result<(), String> {
    let app_clone = app.clone();
    ...
    app.run_on_main_thread(move || {
        if let Some(window) = app_clone.get_webview_window("rebuild-overlay") {
            let _ = window.hide();
            REBUILD_OVERLAY_VISIBLE.store(false, Ordering::SeqCst);
            peek_log!("🔄 Hiding rebuild overlay");
        }
    })
    .map_err(|e| e.to_string())?;
...
/// Returns whether the rebuild overlay is currently visible
pub fn is_rebuild_overlay_visible() -> bool {
    REBUILD_OVERLAY_VISIBLE.load(Ordering::SeqCst)
}
