...
/// `prod`/`production`) supplies no build ID. Development rebuilds therefore
/// share one identity; developers swap helpers with the explicit
/// Disable/Grant workflow.
...
/// embeds an empty ID: packaged builds (`NIXMAC_ENV` = `prod`/`production`,
/// not the cargo profile, which is `release` even for local sidecar builds)
/// fail when the value is missing or empty, and development builds fall back
/// to the one fixed [`DEVELOPMENT_BUILD_ID`] literal.
pub fn resolve_build_id(raw: Option<&str>, packaged: bool) -> Result<String, String> {
    match raw.filter(|value| !value.is_empty()) {
        ...
        None if packaged => Err(
            "NIXMAC_BUILD_ID must be a non-empty value for packaged builds (NIXMAC_ENV=prod); CI supplies it from the packaged source revision"
                .to_string(),
        ),
