...
pub async fn handle_evolve_command(app: &AppHandle, cfg: EvolveConfig) -> Result<(), String> {
    ...
    if let Some(out_path) = out {
        let state_str = evolution_result_state(&output_value, ok);
...
/// The evolution's lifecycle state as reported in the result envelope, so
/// consumers can branch on it without digging into result.telemetry.state.
/// Both `EvolutionResult` and `EvolutionFailureResult` carry it at
/// `telemetry.state`; the fallback only covers outputs that failed to
/// serialize into that shape.
...
mod tests {
    ...
    #[test]
    fn evolution_result_state_should_report_limit_reached_from_telemetry() {
        let output = output_with_state(EvolutionState::LimitReached);
        assert_eq!(evolution_result_state(&output, true), "limitReached");
    }

    #[test]
    fn evolution_result_state_should_report_conversational_from_telemetry() {
        let output = output_with_state(EvolutionState::Conversational);
        assert_eq!(evolution_result_state(&output, true), "conversational");
    }

    #[test]
    fn evolution_result_state_should_report_failed_from_failure_telemetry() {
        let output = output_with_state(EvolutionState::Failed);
        assert_eq!(evolution_result_state(&output, false), "failed");
    }

    #[test]
    fn evolution_result_state_should_fall_back_to_generated_when_telemetry_missing() {
        let output = json!({ "raw": "unserializable output" });
        assert_eq!(evolution_result_state(&output, true), "generated");
    }

    #[test]
    fn evolution_result_state_should_fall_back_to_failed_when_telemetry_missing() {
