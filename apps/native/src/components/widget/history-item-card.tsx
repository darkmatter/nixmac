...
interface HistoryItemCardProps {
  item: HistoryItem;
  onGenerateFrom: (hash: string, count: number) => void;
}

export function HistoryItemCard({ item, onGenerateFrom }: HistoryItemCardProps) {
  ...
  return (
    <div
      className={cn(
        "rounded-[10px] border border-white/10 bg-[#111111] px-[14px] py-3 mb-2 cursor-pointer transition-colors duration-150",
      ...
      tabIndex={0}
    >
      {/* Top row: commit message + restore button */}
      <div className="flex items-start justify-between gap-[10px]">
        <span className="flex-1 text-[13px] font-medium leading-[1.4] text-white">
        ...
        <Button
          ...
          variant="outline"
          size="sm"
          className="h-auto shrink-0 whitespace-nowrap border-white/10 bg-white/[0.06] px-[10px] py-1 text-[10px] text-neutral-400 hover:border-white hover:bg-white hover:text-black"
      ...
      {/* Generate summary — shown only when no summary exists */}
      {!summary && (
        <div className="mt-3">
          <Button
            type="button"
