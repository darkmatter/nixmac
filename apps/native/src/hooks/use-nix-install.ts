...
import type { NixInstallEndEvent, NixInstallProgressEvent } from "@/ipc/types";
...
const checkNix = async () => {
  try {
    ...
    store.setDarwinRebuildAvailable(
      result.installed ? result.darwinRebuildAvailable : null,
    );
  } catch {
...
const installNix = async () => {
  ...
  const unlistenProgress = await ipcRenderer.on<NixInstallProgressEvent>(
  ...
  const unlistenEnd = await ipcRenderer.on<NixInstallEndEvent>(
    "nix:install:end",
    (event) => {
      const current = useWidgetStore.getState();
      ...
      current.setDarwinRebuildAvailable(
        event.payload.darwin_rebuild_available ?? false,
      );
