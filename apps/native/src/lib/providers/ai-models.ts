...
export interface AiModelProvider {
  ...
  setup:
    ...
    | {
        kind: "baseUrl";
        prefsBaseUrlField: "vllmApiBaseUrl";
        prefsKeyField?: "vllmApiKey";
...
export const DEFAULT_EVOLVE_MODEL: Record<string, string> = Object.fromEntries(
  AI_MODEL_PROVIDERS.map((provider) => [provider.id, provider.defaultEvolveModel]),
);

export const DEFAULT_SUMMARY_MODEL: Record<string, string> = Object.fromEntries(
  AI_MODEL_PROVIDERS.map((provider) => [provider.id, provider.defaultSummaryModel]),
);

export function getAiModelProvider(id: string): AiModelProvider {
  return AI_MODEL_PROVIDERS.find((provider) => provider.id === id) ?? AI_MODEL_PROVIDERS[0];
}
