...
DEFAULT_RUNS_DIR: Path = SCRIPT_DIR / "data/runs"
...
def run_test_case(
    ...
    try:
        ...
        try:
            subprocess.run(cmd, check=False, timeout=case_timeout_seconds)
        except subprocess.TimeoutExpired:
            print(
                f"nixmac timed out after {case_timeout_seconds}s for case {case.num}; "
            ...
            return {
                ...
                "success": False,
                "state": "failed",
                "error": f"nixmac timed out after {case_timeout_seconds}s",
                "case": case.num,
                "command": cmd,
                "result": {
                    "error": f"nixmac timed out after {case_timeout_seconds}s",
...
def enrich_result(case: EvalTestCase, result: Any, parsed_args: argparse.Namespace) -> dict[str, Any]:
    """Attach eval metadata needed by the grader/judge without mutating raw internals."""
    enriched = result if isinstance(result, dict) else {"raw_result": result}

    enriched.setdefault("prompt", case.request)
    enriched.setdefault("expected_outcome", case.expected)
    enriched.setdefault("_case_id", case.num)
    enriched["_eval_metadata"] = {
        "case_id": case.num,
        "category": case.feature,
        "subcategory": case.scenario,
        "quality_dimension": case.persona,
        "priority": case.priority,
        "evolve_provider": parsed_args.evolve_provider,
        "evolve_model": parsed_args.evolve_model,
        "summary_provider": parsed_args.summary_provider,
        "summary_model": parsed_args.summary_model,
        "vllm_url": parsed_args.vllm_url,
        "max_iterations": parsed_args.max_iterations,
        "max_build_attempts": parsed_args.max_build_attempts,
    }
    return enriched


def update_test_case_status(case: EvalTestCase, result: Any, output_dir: Path, parsed_args: argparse.Namespace) -> Path:
    ...
    result_path = output_dir / f"case_{case_num}_result.json"
    enriched = enrich_result(case, result, parsed_args)
...
def generate_nixmac_settings(
    ...
    if max_build_attempts is not None:
        settings["maxBuildAttempts"] = max_build_attempts
...
def result_output_dir(parsed_args: argparse.Namespace) -> Path:
    """Resolve where case result JSONs should be written."""
    if parsed_args.run_id:
        return Path(parsed_args.output_root) / parsed_args.run_id
    return RESULTS_DIR
...
def build_manifest(
    ...
    current_selected = [case.num for case in cases]
    current_attempted = [case.num for case in cases if not case.skip]
    selected_cases = sorted({*previous_selected, *current_selected})
    attempted_cases = sorted({*previous_attempted, *current_attempted})
    completed_cases = sorted(
        int(path.stem.split("_")[1])
        for path in output_dir.glob("case_*_result.json")
        if path.stem.split("_")[1].isdigit()
    )
    previous_skipped = previous_manifest.get("counts", {}).get("skipped", 0)
    current_skipped = len([case for case in cases if case.skip])
    return {
        ...
        "nixmac_binary": str(Path(parsed_args.nixmac)),
        "nixmac_binary_sha256": sha256_file(Path(parsed_args.nixmac)),
        "prompt_source": str(csv_path),
        "prompt_source_sha256": sha256_file(csv_path),
        ...
        "counts": {
            ...
            "skipped": max(previous_skipped, current_skipped),
        },
        "providers": {
            "evolve_provider": parsed_args.evolve_provider,
            "evolve_model": parsed_args.evolve_model,
        ...
        "limits": {
            "max_iterations": parsed_args.max_iterations,
            "max_build_attempts": parsed_args.max_build_attempts,
            "case_timeout_seconds": parsed_args.case_timeout_seconds,
        },
        "secrets": {
            "openai_key": "present-redacted" if parsed_args.openai_key else "absent",
            "openrouter_key": "present-redacted" if parsed_args.openrouter_key else "absent",
            "vllm_api_key": "present-redacted" if parsed_args.vllm_api_key else "absent",
        },
        "definitions": {
            "attempted": "selected CSV rows not marked skip=TRUE",
            "completed": "parseable case_<id>_result.json written by the harness",
        },
}
...
def main(parsed_args: argparse.Namespace) -> None:
    ...
    output_dir = result_output_dir(parsed_args)
    ...
    try:
        ...
        write_manifest(build_manifest(parsed_args, cases, output_dir, started_at), output_dir)
        for case in cases:
            if stop_requested:
            ...
            try:
                result = run_test_case(
                    ...
                    parsed_args.vllm_url,
                    parsed_args.vllm_api_key,
                    parsed_args.max_iterations,
                    parsed_args.max_build_attempts,
                    parsed_args.case_timeout_seconds,
                    parsed_args.host,
                )
            ...
            update_test_case_status(case, result, output_dir, parsed_args)
            if stop_requested:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="vLLM OpenAI-compatible base URL (e.g. http://lambda-b200:8000/v1)",
    )
    parser.add_argument(
        "--vllm-api-key",
        dest="vllm_api_key",
    ...
    parser.add_argument(
        ...
        type=int,
        default=25,
        help="Maximum evolve loop iterations to persist in nixmac settings",
    )
    parser.add_argument(
        "--max-build-attempts",
        dest="max_build_attempts",
        type=int,
        default=5,
        help="Maximum build attempts to persist in nixmac settings",
    )
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional timeout for each nixmac evolve subprocess",
    )
    parser.add_argument(
        "--run-id",
        dest="run_id",
        type=str,
        default=None,
        help="Run ID. When set, results write to --output-root/<run-id>",
    )
    parser.add_argument(
        "--output-root",
        dest="output_root",
        ...
        help="Root directory for --run-id output",
    )
