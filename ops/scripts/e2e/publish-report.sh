...
#                     - Optional all-or-none PR-head guard, rechecked per attempt
...
latest_update_allowed() {
	...
	if [[ "$latest_guard_count" == "3" ]]; then
		local current_sha
		if ! current_sha="$(gh api "repos/${LATEST_GUARD_REPOSITORY}/pulls/${LATEST_GUARD_PR}" --jq '.head.sha' 2>/dev/null)"; then
			echo "Unable to recheck the PR head; preserving the immutable report without updating latest." >&2
			return 1
		fi
		if [[ "$current_sha" != "$LATEST_GUARD_EXPECTED_SHA" ]]; then
			echo "The PR head changed; preserving the immutable report without updating latest." >&2
			return 1
		fi
	fi
