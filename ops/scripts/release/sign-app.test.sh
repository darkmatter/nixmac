...
if ! grep -F -- "--identifier com.darkmatter.nixmac.helper --entitlements $REPO_ROOT/apps/native/src-tauri/entitlements-none.plist" "$CODESIGN_LOG" >/dev/null; then
	echo "expected sign-app to sign the helper with its identifier and entitlements" >&2
	cat "$CODESIGN_LOG" >&2
	exit 1
fi

if ! grep -F -- "--identifier com.darkmatter.nixmac.sync-agent --entitlements $REPO_ROOT/apps/native/src-tauri/entitlements-none.plist" "$CODESIGN_LOG" >/dev/null; then
	echo "expected sign-app to sign the sync agent with its identifier and empty entitlements" >&2
	cat "$CODESIGN_LOG" >&2
	exit 1
fi
