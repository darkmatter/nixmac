...
#
# Required env vars:
#   RELEASE_MODE          - "tag" | "develop" | "branch" (from compute-version.sh)
#   RELEASE_VERSION       - computed version (from compute-version.sh, for tag/develop)
...
if [[ "$RELEASE_MODE" == "tag" || "$RELEASE_MODE" == "develop" ]]; then
