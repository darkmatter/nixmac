...
this document — it may change between releases (see
`privileged-helper-reconciliation-mechanics.md`). The guarantees that
procedure must keep are part of it, in §8. Everything in this document —
those guarantees included — may not change.
...
- **The `TryActivate` result reply** arrives on the same connection when the
  ...
  it **without a deadline** — an activation legitimately runs for many
  minutes, and only a peer close or an unparseable reply ends the wait. Read
  deadlines belong on connecting, on `Status`, and on `Retire`; putting one
  here manufactures unknown outcomes out of ordinary long applies. **Within
  this protocol, the result reply is the only way to learn an activation's
...
result arrived, the outcome is unknown: succeeded, failed, or still running.
...
- **8.6 Approval is a state, not an error.** `requiresApproval` means macOS
  ...
  Items, repeatedly if clicked again. No registration churn either way. While
  pending, Apply uses the administrator-password path and the sync agent
...
- **8.11 The password path needs proven quiet.** The administrator-password path
  ...
  nixmac never password-activates while an `enabled` registration could still
  admit sync-agent work — even one the user never granted or has since
  disabled; such a registration is first adopted or removed. Once a helper
  exchange has been attempted, nothing it produces can select the password
...
- **10.3 Concurrent login sessions converge, not coordinate**: a second user's GUI
  ...
  verification, reported and repaired by the next attempt; a different-build
...
- **10.4 The password path is a point-in-time decision**: it is selected from the
  state observed at that moment. nixmac never invalidates its own decision
  (it does not register a helper while its own password activation runs), but
  an independent actor can: a user approving the helper in Login Items
  mid-activation, or a `KeepAlive` relaunch after a positive-absence
...
- **10.9 A wedged helper** — unresponsive, or `Busy(X)` forever because an
  ...
  documented recovery is a reboot, after which reconciliation converges from
  observed state.
