...
function storybookPreviewFromEnv(env = process.env) {
  ...
  if (env.NIXMAC_E2E_STORYBOOK_PREVIEW_JSON) {
    try {
      ...
      return invalidStorybookPreviewMetadata(
        env,
        "Storybook preview metadata was present but not valid JSON.",
      );
    }
  }
...
export async function createBaseState(
  ...
  const sha =
    tryRun("git", ["rev-parse", "--short", "HEAD"], { cwd: repoRoot }).stdout || "unknown";
  const macosVersion =
    env.NIXMAC_E2E_MACOS_VERSION || tryRun("sw_vers", ["-productVersion"]).stdout || "unknown";
...
export function applyHistoricalRenderMigration(state) {
  if (
    state.scenarios.saveFlow?.status === "inconclusive" &&
    state.scenarios.saveFlow.notes.length === 1 &&
    /added after this run/.test(state.scenarios.saveFlow.notes[0])
  ) {
    state.scenarios.saveFlow.notes = [
  ...
  if (state.scenarios.discard?.status === "pass" && !state.safety?.disposableConfig) {
    ...
    state.scenarios.discard.notes.push(
      "Historical pass downgraded for regenerated report: Discard confirmation was not safe to count as pass because disposable config mode was not proven.",
    );
  }
  if (
