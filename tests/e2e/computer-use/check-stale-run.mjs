...
function ghApi(pathname) {
  ...
  if (result.status !== 0)
    return { ok: false, error: result.stderr || result.stdout || `gh api exited ${result.status}` };
  try {
...
export function classifyStaleRun({
  ...
  if (!currentPullRequest) {
    return {
      stale: false,
      reason: apiError ? `api-uncertainty: ${apiError}` : "api-uncertainty",
      prNumber,
      eventHeadSha,
      currentHeadSha: "",
    };
  ...
  if (!currentHeadSha) {
    return {
      stale: false,
      reason: "api-uncertainty: missing current head sha",
      prNumber,
      eventHeadSha,
      currentHeadSha,
    };
...
if (process.argv[2] === "self-test") {
  ...
  main().catch((error) => {
    ...
    writeOutput(
      "reason",
      `api-uncertainty: ${error instanceof Error ? error.message : String(error)}`,
    );
