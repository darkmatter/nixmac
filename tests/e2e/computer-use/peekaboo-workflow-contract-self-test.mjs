...
function assertManualOnlyTrigger(workflowDocument, label = "Legacy Peekaboo workflow") {
  assert.ok(
    workflowDocument.on &&
      typeof workflowDocument.on === "object" &&
      !Array.isArray(workflowDocument.on),
    `${label} on must be a mapping`,
  );
  assert.deepEqual(
    Object.keys(workflowDocument.on),
    ["workflow_dispatch"],
    `${label} must expose exactly one manual workflow_dispatch trigger`,
  );
  assert.ok(
    workflowDocument.on.workflow_dispatch &&
      typeof workflowDocument.on.workflow_dispatch === "object" &&
      !Array.isArray(workflowDocument.on.workflow_dispatch),
...
const workflowDocument = parseWorkflowYaml({
  workflowName: "Peekaboo E2E",
  source: workflow,
});
...
assertManualOnlyTrigger(workflowDocument);
  ...
  assert.throws(
    () =>
      assertManualOnlyTrigger(
        ...
        `Mutated Peekaboo workflow with ${automaticTrigger}`,
      ),
    /exactly one manual workflow_dispatch trigger/,
    `Peekaboo trigger contract must reject ${automaticTrigger}`,
  );
}
assert.equal(
  workflowDocument.on.workflow_dispatch.inputs?.build_attempts?.default,
  "2",
...
assert.match(
  nativeMain,
  /if e2e_webview_watchdog \{[\s\S]*e2e_schedule_webview_boot_probe[\s\S]*NIXMAC_E2E_WEBVIEW_WATCHDOG_SECS[\s\S]*unwrap_or\(12\)[\s\S]*Duration::from_secs\(watchdog_secs\)[\s\S]*run_on_main_thread/,
  "Native app must keep the 12-second bounded reload scheduler inside the explicit E2E watchdog gate",
);
assert.doesNotMatch(
  nativeMain,
...
assert.match(
  nativeMain,
  /run_on_main_thread\(move \|\| \{\n\s+e2e_request_webview_boot_probe\(&reload_window, "watchdog-before-reload"\);\n\s+if let Err\(reload_err\) = reload_window\.reload\(\)/,
  "Watchdog reload must probe the WebView immediately before its sole main-thread reload request",
);
assert.deepEqual(
  rustIdentifierSites(nativeMain, "reload"),
  ["if let Err(reload_err) = reload_window.reload() {"],
  "Native app reload identifiers must remain limited to the sole watchdog invocation",
);
assert.match(
  nativeSecrets,
  /fn get_secret_pref<R: Runtime>\(app: &AppHandle<R>, key: &'static str\) -> Result<Option<String>> \{[\s\S]*if let Some\(store\) = hermetic_secret_store\(key\)[\s\S]*if crate::env::e2e_mock_system_enabled\(\)[\s\S]*get_persistent_secret_pref\(app, key\)/,
  "get_secret_pref must preserve hermetic, E2E mock, and persistent-reader precedence",
);
assert.match(
  debugCommands,
...
assert.match(
  ...
  /export function markBootRenderStage[\s\S]*export function markBootStage/,
  "Frontend boot diagnostics must continue exporting separate render-safe and effect-safe marker APIs",
);
assert.match(
...
assert.match(
  frontendMain,
  /const bootstrap = async \(\) => \{[\s\S]*const telemetry = await initTelemetry\(\);[\s\S]*setTelemetryProvider\(telemetry\);[\s\S]*telemetry\.captureEvent\([\s\S]*renderApp\(telemetry\)/,
  "Frontend bootstrap must install and render with the same initialized telemetry provider",
);
assert.match(
  frontendMain,
  /import\s*\{\s*getTelemetry,\s*setTelemetryProvider\s*\}\s*from\s*"@\/lib\/telemetry\/instance"/,
  "Frontend bootstrap must import the unified telemetry getter and installer",
);
assert.match(
  frontendTelemetryInit,
  /setTelemetryProvider\((?:noopProvider|provider)\)/,
  "Telemetry initialization must install the provider chosen for the current diagnostics policy",
);
assert.match(
  frontendMain,
  ...
  "Frontend bootstrap must report synchronous render-fatal errors through telemetry before rendering the fatal fallback",
);
assert.match(
...
assert.match(
  frontendMain,
  /if\s*\(\s*isE2eProfile\s*\)\s*\{[\s\S]*import\("@\/e2e\/boot-harness"\)[\s\S]*const bootstrap = async/,
  "Frontend boot must queue the E2E harness import before starting telemetry bootstrap",
);
assert.doesNotMatch(
  frontendMain,
