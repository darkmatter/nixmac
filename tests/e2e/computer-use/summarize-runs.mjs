...
function parseArgs(argv) {
  ...
  while (args.length) {
    ...
    if (arg === "--root") options.root = requireValue(arg, args.shift());
      ...
      options.limit = Number.parseInt(requireValue(arg, args.shift()), 10);
    else if (arg === "--release-trailing-clean")
      options.releaseTrailingClean = Number.parseInt(requireValue(arg, args.shift()), 10);
    else if (arg === "--release-latest-sha-clean")
      options.releaseLatestShaClean = Number.parseInt(requireValue(arg, args.shift()), 10);
    else if (arg === "--required-trailing-clean")
      options.requiredTrailingClean = Number.parseInt(requireValue(arg, args.shift()), 10);
    else if (arg === "--required-latest-sha-clean")
      options.requiredLatestShaClean = Number.parseInt(requireValue(arg, args.shift()), 10);
...
function classifyRun(relativeRunDir, state, parseError = null) {
  if (parseError)
    return {
      category: "corrupt-json",
      countsForProduct: false,
      countsForGate: false,
      reason: parseError,
    };
  ...
  if (!state?.verdict || !scenarios) {
    return {
      ...
      countsForProduct: false,
      countsForGate: false,
      reason: "missing verdict or scenario contracts",
    };
  }
  const firstSegment = relativeRunDir.split("/")[0] || "";
  if (/^(chunk\d|baseline-)/.test(firstSegment)) {
    return {
      category: "render-fixture",
      countsForProduct: true,
      countsForGate: false,
      reason: "local copied fixture or render validation directory",
    };
  }
  ...
  if (noTouchUnavailable) {
    return {
      category: "no-touch-unavailable",
      countsForProduct: true,
      countsForGate: false,
      reason: "inconclusive report without app evidence artifacts",
    };
  }
  if (isLiveWorkflow)
    return {
      category: "real-workflow",
      countsForProduct: true,
      countsForGate: true,
      reason: "nested live PR workflow artifact",
    };
  return {
    category: "local-validation",
    countsForProduct: true,
    countsForGate: false,
    reason: "local or historical validation artifact",
  };
}
...
function buildGateEvidence(runs) {
  ...
  const latestShaCleanRuns = latestSha
    ? deduped.filter((run) => run.sha === latestSha && isCleanRun(run)).length
    : 0;
  return {
    statement:
      "Local artifact summaries are presentation evidence only. They do not satisfy required branch protection or release-gate policy by themselves.",
...
function realWorkflowRuns(runs) {
  const realRuns = runs
    ...
    .sort(
      (a, b) =>
        startedAtSortValue(a) - startedAtSortValue(b) ||
        a.relativeRunDir.localeCompare(b.relativeRunDir),
    );
  const deduped = [];
  const seen = new Set();
...
function readinessBlockers({
  ...
  if (trailingClean < trailingThreshold) {
    blockers.push(
      `${label} needs ${trailingThreshold} trailing clean real workflow runs; current local evidence has ${trailingClean}.`,
    );
  }
  if (latestShaClean < latestShaThreshold) {
    blockers.push(
      `${label} needs ${latestShaThreshold} clean real workflow runs on the latest SHA; current local evidence has ${latestShaClean}.`,
    );
  }
  return blockers;
}
...
function buildPromotionReadiness(runs, gateEvidence, thresholds) {
  ...
  const noTouchUnavailableReports = runs.filter(
    (run) => run.classification.category === "no-touch-unavailable",
  );
  ...
  return {
    ...
    advisoryEvidence: {
      ...
      blockers: advisoryReady
        ? []
        : ["Needs at least one clean real workflow Product Proof run in local evidence."],
    },
    releaseHighRisk: {
      state: releaseState,
...
export async function summarizeRuns(options = {}) {
  ...
  const verdictCounts = countBy(productRuns, (run) =>
    VERDICTS.includes(run.verdict) ? run.verdict : "unknown",
  );
  ...
  const sortedRuns = [...runs].sort(
    (a, b) =>
      startedAtSortValue(b) - startedAtSortValue(a) ||
      a.relativeRunDir.localeCompare(b.relativeRunDir),
  );
...
function renderMarkdown(summary) {
  ...
  lines.push(
    `- Real workflow runs counted for clean streak: ${summary.gateEvidence.realWorkflowRuns}`,
  );
  lines.push(
    `- Trailing clean real workflow runs: ${summary.gateEvidence.trailingCleanRealWorkflowRuns}`,
  );
  lines.push(
    `- Latest SHA clean runs: ${summary.gateEvidence.latestShaCleanRuns}${summary.gateEvidence.latestSha ? ` (${shortSha(summary.gateEvidence.latestSha)})` : ""}`,
  );
  lines.push(
    `- Verdicts: ${VERDICTS.map((verdict) => `${verdict} ${summary.verdictCounts[verdict] || 0}`).join(" / ")}`,
  );
  lines.push(
    `- Real workflow evidence: ${summary.realWorkflowEvidence.screenshots.total} screenshots, ${summary.realWorkflowEvidence.textSnapshots.total} text snapshots, ${summary.realWorkflowEvidence.videos.available || 0} videos available`,
  );
  lines.push(
    `- All Product Proof evidence: ${summary.evidence.screenshots.total} screenshots, ${summary.evidence.textSnapshots.total} text snapshots, ${summary.evidence.videos.available || 0} videos available`,
  );
  lines.push("");
  ...
  lines.push(
    `- Release/high-risk gate candidate: ${summary.promotionReadiness.releaseHighRisk.state}`,
  );
  lines.push(
    `- Broad required PR gate candidate: ${summary.promotionReadiness.requiredPrGate.state}`,
  );
  lines.push(
    `- Thresholds: release ${summary.promotionReadiness.thresholds.releaseTrailingClean} trailing / ${summary.promotionReadiness.thresholds.releaseLatestShaClean} latest-SHA clean; required ${summary.promotionReadiness.thresholds.requiredTrailingClean} trailing / ${summary.promotionReadiness.thresholds.requiredLatestShaClean} latest-SHA clean`,
  );
  lines.push(
    `- Infra telemetry: ${summary.promotionReadiness.infraTelemetry.noTouchUnavailableReports} no-touch unavailable reports, ${summary.promotionReadiness.infraTelemetry.nonCleanRealWorkflowRuns} non-clean real workflow runs`,
  );
  ...
  if (summary.gateEvidence.latestRealWorkflowRun) {
    ...
    lines.push(
      `- Result: ${run.scenarios.pass} pass / ${run.scenarios.fail} fail / ${run.scenarios.inconclusive} inconclusive`,
    );
    lines.push(
      `- Evidence: ${run.screenshots} screenshots, ${run.textSnapshots} text snapshots, video ${run.video}`,
    );
    lines.push(
      `- Duration: ${run.durationSeconds == null ? "unavailable" : `${run.durationSeconds}s`}`,
    );
  } else {
...
async function writeFixture(root, relativeRunDir, state, events = null) {
  ...
  if (events)
    await writeFile(path.join(dir, "events.json"), `${JSON.stringify(events, null, 2)}\n`, "utf8");
}
...
function fixtureState({
  ...
  return {
    ...
    screenshots: Array.from({ length: screenshots }, (_, itemIndex) => ({
      path: `screenshots/${itemIndex}.png`,
    })),
    textSnapshots: Array.from({ length: texts }, (_, itemIndex) => ({
      path: `texts/${itemIndex}.txt`,
    })),
...
async function runSelfTest() {
  ...
  try {
    await writeFixture(
      root,
      "live-pr42-100-1/2026-05-02T010000000Z",
      fixtureState({
        verdict: "fail",
    ...
    await writeFixture(
      root,
      "live-pr42-102-1/2026-05-02T030000000Z",
      fixtureState({ verdict: "pass", startedAt: "2026-05-02T03:00:00.000Z", sha: "sha-pass-a" }),
      [{ ts: "2026-05-02T03:04:00.000Z", type: "done" }],
    );
    await writeFixture(
      root,
      "live-pr42-103-1/2026-05-02T040000000Z",
      fixtureState({ verdict: "pass", startedAt: "2026-05-02T04:00:00.000Z", sha: "sha-pass-b" }),
    );
    await writeFixture(
      root,
      "chunk3-render-baseline",
      fixtureState({ verdict: "pass", startedAt: "2026-05-02T04:00:00.000Z", sha: "sha-pass-b" }),
    );
    await writeFixture(
      root,
      "2026-05-02T050000000Z",
      fixtureState({
        verdict: "pass",
        startedAt: "2026-05-02T05:00:00.000Z",
        sha: "sha-local",
        remote: false,
      }),
    );
    await writeFixture(root, "2026-04-30T170340641Z-prompt-only", {
      status: "pass",
      note: "legacy prompt-only state",
    ...
    assert.equal(
      summary.runs.find((run) => run.path === "chunk3-render-baseline").duplicateMetadataGroupSize,
      2,
    );
    assert.equal(
      summary.runs.some((run) => JSON.stringify(run).includes("DXU97120")),
      false,
    );
    assert.equal(
      summary.runs.some((run) => JSON.stringify(run).includes("Add a package")),
      false,
    );
