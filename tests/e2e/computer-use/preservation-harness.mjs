...
const ADVERSARIAL_FIXTURE_FILE = path.join(
  REPO_ROOT,
  "apps/native/src/components/widget/adversarial-new-visible-surface.tsx",
);
const ADVERSARIAL_FIXTURE_CONTENT =
...
function acquireLock() {
  try {
    ...
    const owner = existsSync(path.join(LOCK_DIR, "pid"))
      ? readFileSync(path.join(LOCK_DIR, "pid"), "utf8").trim()
      : "unknown";
    throw new Error(
      `Another preservation harness appears to be running (lock ${LOCK_DIR}, pid ${owner}).`,
    );
  }
...
function buildFixtureRun(workDir) {
  const root = workDir
    ? path.resolve(REPO_ROOT, workDir)
    : mkdtempSync(path.join(os.tmpdir(), "nixmac-preservation-"));
...
function reportSignature(runDir, state) {
  ...
  const nonDynamicScenarios = Object.entries(state.scenarios || {}).filter(
    ([key]) => !DYNAMIC_SCENARIOS.has(key),
  );
  ...
  return {
    ...
    order: {
      ...
      failuresBeforeInconclusive:
        positions.failures >= 0 && positions.inconclusive > positions.failures,
      inconclusiveBeforePassing:
        positions.inconclusive >= 0 && positions.passing > positions.inconclusive,
    },
    ...
    duplicateIds,
    evidenceStrengthCounts: Object.values(state.v2?.scenarioContracts || {}).reduce(
      (counts, item) => {
...
function runRenderExisting(runDir) {
  ...
  if (!existsSync(regeneratedPath))
    throw new Error(`render-existing did not create ${regeneratedPath}`);
  const state = readJson(regeneratedPath);
  if (state.regeneratedFrom !== "state.json")
    throw new Error(`Expected regeneratedFrom=state.json, got ${state.regeneratedFrom}`);
...
function main() {
  ...
  if (command === "update-fixtures" && !args.includes("--allow-rewrite")) {
    throw new Error(
      "update-fixtures rewrites preservation baselines; pass --allow-rewrite after reviewing the intended contract change.",
    );
  }
