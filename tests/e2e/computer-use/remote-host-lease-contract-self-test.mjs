...
const statusHelperPath = path.join(
  repoRoot,
  "ops/runner/macincloud-host-lease-status.py",
);
const recoverHelperPath = path.join(
  repoRoot,
  "ops/runner/macincloud-host-lease-recover.py",
);
const helper = readFileSync(helperPath, "utf8");
const statusHelper = readFileSync(statusHelperPath, "utf8");
const recoverHelper = readFileSync(recoverHelperPath, "utf8");
const leaseSources = [helper, statusHelper, recoverHelper].join("\n");
// The fixture deliberately creates 60-second max-hold heartbeats. Keep the
// harness timeout above that boundary while the helper's own probes and waits
...
assert.match(
  recoverHelper,
  /\["pgrep", "-f", pattern\]/,
  "recovery must probe remote processes",
);
assert.match(recoverHelper, /nixmac.*CuaDriver/s, "recovery must refuse while nixmac is active");
assert.equal(
  [
    ...leaseSources.matchAll(
      /\b(?:ps_path|heartbeat_ps_path)\s*=\s*["']\/bin\/ps["']/g,
    ),
  ].length,
  3,
  "all heartbeat process probes must default to the remote macOS /bin/ps",
);
assert.equal(
  [...leaseSources.matchAll(/\/bin\/ps/g)].length,
  3,
  "all production /bin/ps references must flow through the three audited probe paths",
);
assert.match(
  helper,
...
while (($#)); do
  case "$1" in
    ...
    bash)
      shift
      # OpenSSH sends one space-joined command string to the remote login
      # shell. Reproduce that boundary so empty/raw-whitespace argv bugs cannot
...
`,
  ...
  const invoke = (command, ownerToken, extra = [], env = fixtureEnv) => {
    ...
    if (result.status === null) {
      throw new Error(
        `lease helper ${command} did not exit: ${result.error?.message || result.signal || "unknown failure"}\n${result.stderr || ""}`,
      );
    }
    return result;
  };
  const invokeWithOwnership = (
    command,
    ownerToken,
    ownershipArgs,
    extra = [],
...
stderr=${recoveredRerun.stderr}`,
  ...
  if (process.env.NIXMAC_E2E_STOP_AFTER_RERUN_RECOVERY === "1") {
    throw new Error("NIXMAC_E2E_DIAGNOSTIC_STOP_AFTER_RERUN_RECOVERY");
  }
