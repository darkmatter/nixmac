...
async function main() {
  ...
  if (command === 'end') {
    ...
    const existing = timing.phases.find((phase) => phase.id === id) || { id, label: id, category: 'workflow', source: 'github-actions' };
    const holder = { timing };
    recordTimingPhase(
      holder,
      {
  ...
  if (command === 'markdown') {
    ...
    process.stderr.write(`Timing summary includes ${merged.phases.length} phases; observed total ${formatDuration(merged.phases.reduce((total, phase) => total + (Number(phase.durationMs) || 0), 0))}.\n`);
    return;
  }
