...
assert.match(nativeMain, /document\.documentElement\.dataset\.nixmacE2eCapture = captureMode[\s\S]*html\[data-nixmac-e2e-capture="\$\{captureMode\}"\][\s\S]*requestAnimationFrame\(\(\) => \{\n\s+document\.documentElement\.dataset\.nixmacE2eCapturePaint = "raf";/, 'Native app capture script must set matching capture selectors and a paint marker');
assert.match(nativeMain, /let e2e_solid_capture = e2e_solid_capture_enabled\(\);[\s\S]*let e2e_capture_background = e2e_solid_capture \|\| e2e_opaque_window[\s\S]*transparent\(!e2e_opaque_window\)[\s\S]*if e2e_capture_background \{\n\s+main_window_builder = main_window_builder\s+\.background_color\(tauri::utils::config::Color\(10, 10, 10, 255\)\);[\s\S]*main_window_builder =\s+main_window_builder\.initialization_script\(E2E_CAPTURE_DARK_BACKGROUND_SCRIPT\);/, 'Native app must keep default solid capture CSS-backed and transparent while limiting native opacity to opaque debug mode');
