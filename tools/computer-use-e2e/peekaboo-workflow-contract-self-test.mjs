...
assert.match(nativeMain, /e2e_page_load_native_capture_probe[\s\S]*native-page-load-finished-plus-5s[\s\S]*e2e_schedule_native_webview_capture_probe/, 'Native app must schedule native WebView capture diagnostics after page-load');
...
assert.match(nativeMain, /e2e_schedule_native_webview_snapshot[\s\S]*page-load-finished-plus-5s[\s\S]*e2e_start_native_webview_snapshot_request_poller/, 'Native app must take bounded page-load readiness snapshots and start the on-demand snapshot request poller only in E2E capture mode');
