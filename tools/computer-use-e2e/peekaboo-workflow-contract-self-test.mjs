...
assert.match(nativeMain, /fn e2e_preload_native_capture_enabled\(\) -> bool \{\n\s+cfg!\(debug_assertions\) && crate::e2e_runtime::enabled\("NIXMAC_E2E_PRELOAD_NATIVE_CAPTURE"\)/, 'Native app must keep pre-load native capture diagnostics behind an explicit E2E-only opt-in gate');
assert.match(nativeMain, /fn e2e_page_load_native_capture_enabled\(\) -> bool \{\n\s+cfg!\(debug_assertions\) && crate::e2e_runtime::enabled\("NIXMAC_E2E_PAGE_LOAD_NATIVE_CAPTURE"\)/, 'Native app must keep page-load native capture diagnostics behind an explicit E2E-only opt-in gate');
assert.match(nativeMain, /let e2e_page_load_native_capture_probe =\s+e2e_css_capture && e2e_page_load_native_capture_enabled\(\);/, 'Native app must not schedule page-load native WebView captures by default on MacInCloud');
assert.match(nativeMain, /if e2e_page_load_native_capture_probe \{[\s\S]*e2e_schedule_native_webview_capture_probe\([\s\S]*native-page-load-finished-plus-5s[\s\S]*e2e_schedule_native_webview_snapshot\([\s\S]*page-load-finished-plus-5s/, 'Native app must only schedule native WebView capture diagnostics after page-load when explicitly opted in');
assert.match(nativeMain, /else if e2e_css_capture \{[\s\S]*NIXMAC_E2E_PAGE_LOAD_NATIVE_CAPTURE disabled/, 'Native app must log when page-load native captures are disabled by default');
assert.match(nativeMain, /if e2e_preload_native_capture_enabled\(\) \{[\s\S]*native-post-build-plus-2s[\s\S]*post-build-plus-2s[\s\S]*NIXMAC_E2E_PRELOAD_NATIVE_CAPTURE disabled/, 'Native app must not run pre-load native WebView captures by default on MacInCloud');
...
assert.match(nativeMain, /e2e_start_native_webview_snapshot_request_poller[\s\S]*NIXMAC_E2E_PAGE_LOAD_NATIVE_CAPTURE disabled/, 'Native app must start the on-demand snapshot request poller while leaving page-load native snapshots opt-in');
...
assert.match(peekabooShell, /peekaboo_window_id_for_app\(\)[\s\S]*window list --app "\$app" --json[\s\S]*\.data\.windows\[\]\?[\s\S]*\.window_id[\s\S]*peekaboo_capture_window_image\(\)[\s\S]*image --mode window --window-id "\$window_id" --path "\$path"/, 'Peekaboo helpers must expose a window-id capture path before falling back to full-screen pixels');
assert.match(productProof, /nixmac_pp_capture_window_visual_signal\(\)[\s\S]*peekaboo_capture_window_image "\$NIXMAC_APP_NAME" "\$path"[\s\S]*nixmac_pp_screenshot_has_visual_signal "\$path"/, 'Product Proof must only use Peekaboo window-id captures after the strict visual-signal probe passes');
assert.match(productProof, /nixmac_pp_capture_ready_visual_signal\(\)[\s\S]*nixmac_pp_capture_window_visual_signal "\$label"[\s\S]*nixmac_pp_capture_native_visual_signal "\$label"/, 'Ready-shell visual proof must try system pixels, then window-id pixels, then native WebView diagnostics');
...
assert.match(productProof, /passed\|rendered\)[\s\S]*printf '%s\\n' "\$output_path"/, 'Product Proof may consume rendered PDF fallback PNGs only after the shell visual-signal probe validates their pixels');
...
assert.match(nativeMain, /let configuration: \*mut Object = std::ptr::null_mut\(\);[\s\S]*takeSnapshotWithConfiguration: configuration/, 'Native snapshot requests must use WebKit full-view snapshots so virtualized hosts do not fail on stale AppKit rect configuration');
assert.match(nativeMain, /fn e2e_try_pdf_snapshot[\s\S]*WKPDFConfiguration[\s\S]*createPDFWithConfiguration: configuration/, 'Native snapshot requests must try WebKit PDF rendering before falling back to black-prone AppKit cached display on virtualized hosts');
assert.match(nativeMain, /"rendered"[\s\S]*WKWebView PDF fallback wrote PNG and PDF; caller must validate visual signal before using it as proof[\s\S]*WKWebView\.createPDFWithConfiguration/, 'Native snapshot PDF fallback status must preserve WebKit PDF provenance without marking unverified PDF pixels as pass-grade proof');
...
assert.match(productProof, /status=\$\(jq -r '\.status \/\/ ""' "\$status_path"[\s\S]*passed\|rendered\)[\s\S]*\[ -s "\$output_path" \][\s\S]*degraded\)[\s\S]*return 1[\s\S]*failed\)[\s\S]*return 1/, 'Product Proof native snapshot reader must wait for terminal status and accept only pass-grade or externally-validated rendered PNG candidates before the visual-signal probe decides proof quality');
