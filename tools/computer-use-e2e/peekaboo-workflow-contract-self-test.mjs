...
assert.match(nativeStore, /fn get_secret_pref[\s\S]*if e2e_mock_system_enabled\(\) \{[\s\S]*return get_string_pref_raw\(app, key\);[\s\S]*load_secret_blob_state/, 'E2E mock-system mode must bypass keychain reads in UI preference secret lookups');
...
assert.match(frontendBootDiagnostics, /function markNativeBootStage\(stage: string\)[\s\S]*tauriAPI\.debug\.markBootStage\(stage, Date\.now\(\)\)[\s\S]*markNativeBootStage\(normalizedStage\)/, 'Frontend boot diagnostics must mirror boot stages to the native debug command before the event-loop stall boundary');
