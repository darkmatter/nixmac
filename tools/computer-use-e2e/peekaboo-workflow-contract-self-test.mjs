...
assert.match(frontendMain, /E2E_MODE = import\.meta\.env\.VITE_NIXMAC_E2E_MODE === "true"[\s\S]*Sentry init skipped for E2E boot[\s\S]*return;/, 'Frontend boot must use the build-time E2E mode flag to skip boot-time Sentry prefs IPC without adding another IPC gate');
