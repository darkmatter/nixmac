...
assert.match(frontendBootDiagnostics, /import \{ sanitizeDiagnosticText \} from "@\/lib\/sentry\/sanitize"/, 'E2E boot diagnostics must consume sanitization from the shared sentry/sanitize module rather than a duplicate regex set');
...
assert.match(frontendMain, /E2E_MODE = import\.meta\.env\.VITE_NIXMAC_E2E_MODE === "true"/, 'Frontend main must derive E2E_MODE from the build-time Vite flag');
...
assertOrder(frontendMain, 'attachSentry();', 'renderApp();', 'Frontend boot must call attachSentry before rendering so the app-mounted Sentry trigger is registered');
assertOrder(frontendMain, 'window.addEventListener(\n  "nixmac:app-mounted"', 'renderApp();', 'Frontend boot must register the heartbeat-stop app-mounted listener before rendering the app');
